...
public abstract class AbstractController implements ApplicationEventPublisherAware {
    ...
    Counter http400ExceptionCounter = Metrics.counter("com.rollingstone.CustomerController.HTTP400");

    Counter http404ExceptionCounter = Metrics.counter("com.rollingstone.CustomerController.HTTP404");

    Counter http500ExceptionCounter = Metrics.counter("com.rollingstone.CustomerController.HTTP500");
    ...
    public @ResponseBody RestAPIExceptionInfo handleISEException(Exception ex,
                                                                              WebRequest request, HttpServletResponse response) {
        log.info("Received Recource Not Found Exception" + ex.getLocalizedMessage());
        http500ExceptionCounter.increment();
        return new RestAPIExceptionInfo(ex.getLocalizedMessage(), "There is an Internal Server Error, Please contact Admin.");
    }
